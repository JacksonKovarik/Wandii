...
export default function FriendsSectionList({ sections }) {
  ...
  const [activeLetter, setActiveLetter] = useState("A"); // tracking bar
  const [dragLetter, setDragLetter] = useState(null);    // index highlight
  ...
  const handleDrag = (e) => {
    ...
    setDragLetter(letter);   // highlight only while dragging
    setActiveLetter(letter); // update tracking bar
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      {/* ALWAYS-VISIBLE TRACKING BAR */}
      <View style={styles.trackingBarContainer}>
        <Text style={styles.trackingBarLetter}>{activeLetter}</Text>
      </View>
      ...
      {!search && (
        <View style={styles.rightIndexContainer} pointerEvents="box-none">
          <View
            ...
            onResponderRelease={() => setDragLetter(null)} // <-- remove highlight
          >
            {ALPHABET.map((letter) => (
              <View key={letter} style={styles.letterRow}>
                <View
                  style={[
                    styles.circle,
                    dragLetter === letter && styles.circleActive, // only while dragging
                  ]}
                />
