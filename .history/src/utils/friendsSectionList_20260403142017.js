...
export default function FriendsSectionList({ sections }) {
  ...
  useEffect(() => {
    ...
    const firstChar = search[0].toUpperCase();

    const index = filteredSections.findIndex(s => s.title === firstChar);
    ...
    <View style={{ flex: 1 }}>
      ...
      {filteredSections.length === 0 && (
        <View style={{ padding: 20, alignItems: "center" }}>
          <Text style={{ fontSize: moderateScale(16), color: "#999" }}>
            No results found
          </Text>
        </View>
      )}
      ...
      <SectionList
        ...
        contentContainerStyle={{
          paddingTop: filteredSections.length > 0 ? TRACKING_BAR_HEIGHT : 0,
