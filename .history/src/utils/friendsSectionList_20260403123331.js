    ...
    <View style={{ flex: 1 }}>
      ...
      <SectionList
        ...
        contentContainerStyle={{
          ...
          paddingRight: scale(30), // prevents overlap with index
        }}
      ...
      <View
        ...
        onResponderRelease={() => {
          setActiveLetter(null); // remove highlight when finger lifts
        }}
      >
        {ALPHABET.map((letter) => (
          <Pressable
            key={letter}
            android_ripple={{ color: "transparent" }} // disable gray ripple
            onPressIn={() => setActiveLetter(letter)}
            onPressOut={() => setActiveLetter(null)}
            onPress={() => scrollToLetter(letter, true)}
            style={[
...
const styles = StyleSheet.create({
  ...
  activeCircle: {
    backgroundColor: "rgba(255, 136, 32, 0.18)", // light orange
