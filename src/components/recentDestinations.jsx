      ...
      <View style={styles.list}>
        {recent.length > 0 ? (
          recent.map((place, index) => {
            const [city, country] = String(place || '').split(', ');

            return (
              <View key={`${place}-${index}`} style={styles.card}>
                <Ionicons name="location-sharp" size={20} color="#FF8820" />

                <View style={{ marginLeft: 10 }}>
                  <Text style={styles.city}>{city || place}</Text>
                  <Text style={styles.country}>{country || 'Saved destination'}</Text>
                </View>
              </View>
            );
          })
          ...
          <View style={styles.emptyCard}>
            <Text style={styles.emptyText}>Destinations from your trips will show up here.</Text>
          </View>
        )}
      </View>
    </View>
